...
describe("isFinished", () => {
  ...
  it("returns false for an initial state  in RF", () => {
    expect(isFinished(rfBoard, 0, rfConfig.royalFavor)).toBeFalsy();
  });
  it("return true if all piles have been completed  in RF", () => {
    expect(
      isFinished({ ...rfBoard, piles: fullPiles }, 0, rfConfig.royalFavor),
    ).toBeTruthy();
  });
  it("return true if there are no fuse tokens left in RF", () => {
    expect(
      isFinished({ ...rfBoard, fuseTokens: 0 }, 0, rfConfig.royalFavor),
    ).toBeTruthy();
  });
